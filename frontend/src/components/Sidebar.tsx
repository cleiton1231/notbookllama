...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-72 h-full bg-[#141414] border-r border-white/[0.08] flex flex-col shrink-0 select-none text-zinc-300">
      ...
      <div className="px-3 pt-2 pb-1 border-b border-white/[0.06] flex items-center space-x-1 bg-white/[0.02]">
        <button
          type="button"
          onClick={() => setActiveTab('documents')}
          className={`flex-1 flex items-center justify-center space-x-1.5 py-1.5 px-2 rounded-lg text-xs font-medium transition ${
            activeTab === 'documents'
              ? 'bg-white/[0.1] text-white shadow-sm'
              : 'text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.04]'
          }`}
        >
          <Files className="w-3.5 h-3.5 text-coral-400" />
          <span>Documentos</span>
          {documents.length > 0 && (
            <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-white/[0.08] text-zinc-300 font-mono">
              {documents.length}
            </span>
          )}
        </button>
        ...
        >
          <MessageSquare className="w-3.5 h-3.5 text-coral-400" />
          <span>Histórico</span>
        </button>
      </div>
      ...
      <div className="p-3 border-t border-white/[0.06] text-[11px] text-zinc-500 flex items-center justify-between font-mono">
        <span className="flex items-center gap-1.5">
          ...
          <span>{activeTab === 'history' ? 'Histórico SQLite' : 'ChromaDB + BM25'}</span>
        </span>
      </div>
    </aside>
  );
